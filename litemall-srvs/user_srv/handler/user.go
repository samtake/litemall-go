...
import (
	...
	"litemall-srvs/user_srv/model"
	proto2 "litemall-srvs/user_srv/proto"
	"strings"
	"time"
)
...
func ModelToRsponse(user model.User) proto2.UserInfoResponse {
...
func (s *UserServer) GetUserList(ctx context.Context, req *proto2.PageInfo) (*proto2.UserListResponse, error) {
...
func (s *UserServer) UpdateUser(ctx context.Context, req *proto2.UpdateUserInfo) (*empty.Empty, error) {
	var user model.User
	result := global.DB.First(&user, req.Id)
	if result.RowsAffected == 0 {
		return nil, status.Errorf(codes.NotFound, "用户不存在")
